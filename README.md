...
현재 확인된 주민 AI 런타임 검사는 로컬 계약 서버를 사용한 `MOCK_CONTRACT`입니다. 이 모드는 Unity의 실제 HTTP 요청·응답 검증·도로 이동을 검사하지만 OpenRouter 제공자를 호출하지 않습니다. 작업 환경에 유효한 키가 없어 실제 `LIVE_OPENROUTER` 호출은 아직 검증하지 않았습니다.
...
현재 v0.6 소스와 Windows 빌드에서 확인한 결과는 다음과 같습니다.

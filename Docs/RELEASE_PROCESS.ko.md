...
버전은 `Assets/Editor/BuildAutomation.cs`의 현재 Player 버전과 일치시킨다. 아래 예시는 `0.6.1`이다.
...
```powershell
.\Tools\Package-Release.ps1 -Version 0.6.1
```
...
.\Tools\Package-Release.ps1 -Version 0.6.1 `
  ...
  -MoviePath .\Artifacts\ResidentActivitySmoke\resident-activity.mp4
```

출력:

- `Builds\Riverworks-v0.6.1-Windows-x64.zip`
- `Builds\Riverworks-v0.6.1-Windows-x64.zip.sha256`
- `Builds\Riverworks-Resident-Gateway-v0.6.1.zip`
- `Builds\Riverworks-Resident-Gateway-v0.6.1.zip.sha256`
...
.\Tools\Verify-Release.ps1 `
  -ZipPath .\Builds\Riverworks-v0.6.1-Windows-x64.zip `
